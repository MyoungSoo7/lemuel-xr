...
- **「잴 것이 없어서 통과」를 조심할 것** — 이 앱의 누르는 것 상당수는 백엔드가 응답해야
  ...
  스위트는 계약을 검증하지 않는다.
- **미션 씬 터치 타깃은 CI 가 지켜 주지 않는다** — `tests/e2e/mission-tap-targets.spec.ts`
  ...
  다만 이 스펙은 **백엔드가 있어야 돌기 때문에 CI 프론트 잡에 없다.** 백엔드를 띄운
  곳에서 돌리지 않으면 그 세 군데는 보호받지 못한다. 분기도 첫 버튼을 따라간 한
  갈래만 본다.
