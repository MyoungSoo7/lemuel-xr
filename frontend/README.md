...
- **모바일은 레이아웃만 잰다** — 2026-08-12 부터 Playwright 에 모바일 프로젝트 3개
  ...
  여기에 `tests/e2e/tap-targets.spec.ts` 가 터치 타깃 44px(WCAG 2.5.5 / Apple HIG)를
  검사한다. 두 스위트 모두 백엔드 없이 판정되고 **CI 의 frontend 잡에서 프로덕션
  빌드에 대고 돈다**(2026-08-12). 개발 서버로 재면 안 된다 — `/david` 는 dev 에서만
  초록이고 프로덕션 빌드에서 측정 결함이 드러난 전례가 있다.
  다만 재는 것은 **레이아웃**이지 성능이 아니다 — 프레임률·로드 시간·R3F 씬의 모바일
  GPU 부하는 여전히 **한 번도 재지 않았다.**
- **「잴 것이 없어서 통과」를 조심할 것** — 이 앱의 누르는 것 상당수는 백엔드가 응답해야
  그려진다(`/topics/journal` 의 감정 칩은 `catalog?.catalog ?? []`). 백엔드 없이 도는
  `tap-targets.spec.ts` 는 그 자리에서 버튼을 0 개 세고, 0 개면 「44px 미만이 없다」가
  참이라 **초록이 된다.** 2026-08-12 에 백엔드를 띄우고 같은 검사를 돌리자 12 개가
  빨개졌다. 그래서 `tests/e2e/tap-targets-data.spec.ts` 를 뒀다 — 응답을 가로채 채워
  넣고 재며, **픽스처가 화면에 닿았는지 먼저 확인하고 분모(전체 타깃 수)까지 본다.**
  새 검사를 붙일 때 이 두 가지를 빠뜨리면 같은 함정에 다시 빠진다.
  픽스처는 실제 응답을 받아 적은 사본이라 **서버 계약이 바뀌면 같이 낡는다** — 이
  스위트는 계약을 검증하지 않는다.
- **미션 씬 터치 타깃 — 2026-08-13 부터 CI 가 지킨다** — `tests/e2e/mission-tap-targets.spec.ts`
  가 7인물 씬을 넘겨 가며 재고, 2026-08-12 에 처음 돌려 세 군데를 고쳤다
  (`/elijah` 위기 확인 씬의 상담번호 링크 **26×17** — 앱이 위험 신호를 판단한 화면에서
  전화 거는 링크가 앱 전체에서 제일 작았다 / `/moses` 68×30 / `/solomon` 36×26).
  이 스펙은 백엔드가 있어야 돌기 때문에 오래 CI 밖에 있었다. 지금은 전용 잡
  `mission-e2e` 가 pgvector 서비스 컨테이너 + `bootRun` 으로 백엔드를 띄우고 돌린다.
  분기는 여전히 첫 버튼을 따라간 한 갈래만 본다.
  **위 「잴 것이 없어서 통과」의 재발이 바로 여기 있었다** — 스펙의 방어가 분모 하나였는데,
  백엔드를 내리고 돌려 보니 7인물이 전부 초록이었다(2026-08-13 실측). 백엔드가 없으면
  `SceneBootState` 의 「세션을 시작하지 못했습니다」 화면과 `다시 시도` 버튼(44px 준수)이
  분모를 채운다. 지금은 `/api/game/{인물}/start` 가 200 으로 왔는지를 네트워크에서
  직접 확인한 뒤에 잰다. 새 검사를 붙일 때 **분모만으로는 부족하다** 는 사례다.
