...
describe("다윗 미션 모놀로그", () => {
  it("Scene 1 도입부가 시 23:1 을 인용한다", () => {
    ...
    expect(scene1Psalm23.trim().length).toBeGreaterThan(0);
    expect(scene1Psalm23).toContain("시 23:1");
  });
  ...
  it("Scene 5 는 단일 나아감(throw) 하나만 갖는다", () => {
    ...
    expect(scene5Monologue.throw).toContain("삼상 17:47");
  });
  ...
  describe("돌(감정) → 결말 매핑이 전사(全射)다", () => {
    it("yml 의 다섯 돌 전부가 echo 텍스트와 결말 텍스트를 갖는다", () => {
      ...
      for (const id of YML_STONE_IDS) {
        expect(scene4LastStone[id], `${id} echo 없음`).toBeTruthy();
        expect(scene6OutroByLastStone[id], `${id} 결말 없음`).toBeTruthy();
      }
  ...
  describe("lastStoneOf", () => {
    ...
    it("빈 배열·전부 잡값이면 trust 로 떨어진다", () => {
      ...
      expect(scene6OutroByLastStone[lastStoneOf([])]).toBeTruthy();
    });
  });

  describe("결말 텍스트의 성질", () => {
    it("다섯 결말이 서로 다르다", () => {
      const outros = Object.values(scene6OutroByLastStone);
      expect(new Set(outros).size).toBe(outros.length);
    });

    it("모든 결말이 본문 인용으로 시작한다", () => {
      for (const [key, text] of Object.entries(scene6OutroByLastStone)) {
    ...
    it("모든 결말이 '일기 한 줄' 로 라우팅한다 — R1 안전 경로", () => {
      ...
      for (const [key, text] of Object.entries(scene6OutroByLastStone)) {
        expect(text, `${key} 결말에 일기 라우팅 없음`).toMatch(/한 줄/);
      }
    });
  });
});
