 ...
 *
 * 위 목록은 각 카드가 딛고 선 **단락(pericope)** 이고, 아래 `q()` 의 표기는 화면이
 * 실제로 읽는 **절** 이다 — 둘이 다를 수 있다. ③ 은 4:2~5 단락에서 4:2·4:3 만,
 * ⑤ 는 4:14~16 단락에서 4:14·4:15 만 읽는다(⑤ 는 §3.4 의 진노 어휘 회피 결정 때문에
 * 4:14 의 앞머리부터 잘라 낸다). 표기를 단락에 맞춰 넓히면 화면이 읽지도 않는 절을
 * 인용했다고 말하게 된다 — 2026-08-22 이전 표기가 그랬다. 목록은 설계, 표기는 실측.
 */
export const scene3CardResponses: Record<Scene3Card, Monologue> = {
  ...
  trust_me: [
    q("출 4:2~3", ["ex-4:2", 3, 8], ["ex-4:3", 2, 5]),
  ...
  send_other: [
    q("출 4:14~15", ["ex-4:14", 7, 12], ["ex-4:15", 8, 19]),
