...
describe("모세 미션 모놀로그", () => {
  it("Scene 1~6 이 빠짐없이 텍스트 소스를 갖는다", () => {
    ...
    expect(scene1Wilderness.trim().length).toBeGreaterThan(0);
  ...
  describe("키 집합이 yml / 페이지 계약과 일치한다", () => {
    ...
    it("Scene 5 는 lift_staff 단일 몸짓", () => {
      ...
      expect(scene5Monologue.lift_staff).toContain("출 14:21");
    });
  ...
  describe("scene3AssignmentsToPattern — 어떤 배정도 결말에 도달한다(전사)", () => {
    ...
    it("전부 throw / 전부 heart / 섞임 3패턴이 모두 도달 가능하다", () => {
      ...
      for (const pattern of ["all_throw", "all_heart", "mixed"] as const) {
        expect(scene3Outcomes[pattern]).toBeTruthy();
        expect(scene6OutroByScene3[pattern]).toBeTruthy();
      }
    });
  ...
  describe("buildScene3Echo — 내려놓은 카드마다 본문이 붙는다", () => {
    it("throw 한 카드의 응답만, yml 카드 순서대로 이어 붙인다", () => {
      ...
      expect(echo.startsWith(scene3Outcomes.mixed)).toBe(true);
      expect(echo).toContain(scene3CardResponses.who_am_i);
      expect(echo).toContain(scene3CardResponses.send_other);
      expect(echo).not.toContain(scene3CardResponses.trust_me);
      expect(echo.indexOf(scene3CardResponses.who_am_i)).toBeLessThan(
        echo.indexOf(scene3CardResponses.send_other),
      );
    });
    ...
    it("다섯 장을 다 내려놓으면 다섯 응답이 다 붙는다", () => {
      ...
      for (const id of YML_CARD_IDS) {
        expect(echo, `${id} 응답 누락`).toContain(scene3CardResponses[id]);
      }
    });
    ...
    it("하나도 내려놓지 않으면 대표 응답(정체성)만 붙는다 — 과부하 방지", () => {
      ...
      expect(echo).toContain(scene3CardResponses.who_am_i);
      expect(echo).not.toContain(scene3CardResponses.cant_speak);
    });
    ...
    it("알 수 없는 카드 id 는 무시하고 대표 응답으로 떨어진다", () => {
      ...
      expect(echo.startsWith(scene3Outcomes.mixed)).toBe(true);
      expect(echo).toContain(scene3CardResponses.who_am_i);
      expect(echo).not.toContain("undefined");
    });
    ...
    it("에코는 언제나 결과 텍스트 + 빈 줄 + 응답 구조다", () => {
      ...
      expect(echo).toBe(
        `${scene3Outcomes.all_throw}\n\n${scene3CardResponses.who_am_i}`,
      );
    });
  });
  ...
  describe("결말 텍스트의 성질", () => {
    it("세 결말이 서로 다르다", () => {
      const outros = Object.values(scene6OutroByScene3);
    ...
    it("모든 결말이 출 3:12 동행 약속을 공통 축으로 갖는다", () => {
      ...
      for (const [key, text] of Object.entries(scene6OutroByScene3)) {
    ...
    it("카드 응답 다섯 개가 각각 본문 인용을 갖는다", () => {
      for (const [key, text] of Object.entries(scene3CardResponses)) {
        expect(text, `${key} 응답에 성경 인용 없음`).toMatch(SCRIPTURE_REF);
      }
    });
  });
});
