...
- `base/` = GREEN 기준선. **전 게이트**(2026-08-18 현재 29개) PASS 여야 러너가 케이스 검사를 시작한다.
  base 가 빨가면 케이스는 **한 건도 안 돈다** — 그리고 그 상태로 몇 달을 살았다(아래 CI 항 참조).
...
- RED 가 vacuous 해지는 두 경로를 둘 다 막는다:
  ...
  2. **델타 검사** — RED 케이스에서 비-PASS 인 게이트 집합이 정확히 `{그 게이트}` 여야 한다.
     ...
     불가피한 경우만 `delta_check: false` + `delta_note` 로 명시 면제한다(3건).
- **하한(래칫)** — 게이트 29개 · 케이스 59개 미만이면 빨강이다(`MIN_GATES`·`MIN_CASES`).
  검사 대상이 사라지는 것은 통과가 아니다. 게이트를 늘리면 이 수도 같이 올린다.

### CI 에서 돈다 (2026-08-18 연결)

`ci.yml` 의 `gate-selftest` 잡. **그 전까지는 사람이 기억해서 로컬에서 돌려야만 도는
검사였고, 그래서 돌지 않았다** — 2026-08-18 에 열어 보니 `base` 가 `G2v` 에서 중단돼
59개 케이스가 통째로 안 돌고 있었다. 아무도 몰랐다. 이 잡은 `gate-baseline` 과 달리
**진짜 합격·불합격**이다.
