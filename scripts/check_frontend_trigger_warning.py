...
────────────────────────────
  ...
  · 화면이 없는 인물. NOTE 로 남기고 건너뛴다. **건너뛴 건 통과가 아니다.**
    2026-08-20 기준 건너뛰는 인물은 없다 — 룻이 마지막이었고(백엔드 enum 은 열렸는데
    `frontend/src/app/ruth/` 가 없어 이 판정기가 조용히 지나쳤다), 화면이 생기면서
    사정권에 들어왔다. NOTE 가 다시 뜨면 그건 "괜찮다"가 아니라 "안 쟀다"는 뜻이다.
