...
def line_with(*needles: str, without: str = "") -> str:
...
F65_STIGMA = line_with("| `rahab_f65_stigma`", "카드 하단(동의 전)")
F65_CLOSE = line_with("| `rahab_f65_close`", "Scene 5 종결 화면 하단(정지 후)")
# 카드 본문 쪽 footer — §5-4 표 행과 문안이 같아 `without="|"` 로 갈라낸다.
# 낙인 카드가 아니라 **강요 카드**를 쓰는 이유: 낙인 카드의 footer 는 두 줄로 접혀 있어
# 줄 단위 변이가 반쪽만 지운다. 한 줄로 끝나는 자리를 골라야 변이가 겨냥한 대로 든다.
CARD_COERCION_FOOT = line_with("당신에게 일어난 일은 당신의 잘못이 아닙니다", without="|")

# (무너뜨리는 축, 겨냥한 검사 키, 설명, before, after)
MUTANTS: list[tuple[str, str, str, str, str]] = [
    ("행 수", "a-rows", "자막 24행에서 한 행을 지운다", CAP_2_1, ""),
    ("좌표 1:1", "a-used", "자막 좌표를 USED 에 없는 절로 바꾼다",
     CAP_2_9, CAP_2_9.replace("수 2:9 중", "수 2:10 중")),
    ("`중` 표기", "a-jung", "부분 인용에서 `중` 을 뗀다",
     CAP_2_9, CAP_2_9.replace("`수 2:9 중`", "`수 2:9`")),
    ("Scene 순서", "a-order", "Scene 안의 순서 번호를 건너뛴다",
     CAP_2_9, CAP_2_9.replace("| 1     | 2    |", "| 1     | 9    |")),
    ("화자 표기", "a-attr", "🚨 2:18 의 화자 표기를 지운다 — R2 오독 경로",
     CAP_2_18, re.sub(r"`정탐꾼`", "—", CAP_2_18, count=1)),
    ("표기 근거", "a-word", "본문에 없는 낱말로 화자를 표기한다",
     CAP_2_18, re.sub(r"`정탐꾼`", "`두 밀정`", CAP_2_18, count=1)),
    ("footer 종수", "f-rows", "§5-4 에서 한 종을 지운다", F65_CLOSE, ""),
    ("카드 집합", "f-cards", "footer 가 없는 카드를 가리키게 한다",
     F65_STIGMA, F65_STIGMA.replace("`rahab_stigma` 카드", "`rahab_ghost` 카드")),
    ("부착 커버리지", "f-cover", "🚨 카드 본문에서 footer 줄을 지운다 — rev.5·rev.7 이 두 번 틀린 자리",
     CARD_COERCION_FOOT, ""),
    ("위기 자원 토큰", "f-token", "footer 에서 `{{crisis_resources.default}}` 를 뗀다",
     F65_CLOSE, F65_CLOSE.replace(" {{crisis_resources.default}}", "")),
    ("토큰 중복", "f-once", "카드 본문에 위기 자원 줄을 하나 더 넣는다",
     CARD_COERCION_FOOT, CARD_COERCION_FOOT + "\n    지금 힘드시면 {{crisis_resources.default}}"),
    ("금지 토큰", "f-forbid", "footer 문안에 금지 토큰을 넣는다",
...
def run(path: Path) -> tuple[int, set[str]]:
    p = subprocess.run([sys.executable, str(CHECK), "--seed", str(path)],
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as d:
        docs = Path(d) / "docs"
        docs.mkdir()
        for axis, key, why, before, after in MUTANTS:
    ...
    if caught != len(MUTANTS):
        ...
        return 1
    print("⚠️ 이 초록이 말하지 않는 것: **잡지 않는 결함의 범위**다. 변이 12종은 판정기가")
    print("   재겠다고 선언한 축의 목록이지 결함 공간의 표본이 아니다. 그리고 `a-yml`·")
    print("   `f-yml` 두 축은 `content/rahab/` 이 없어 **한 번도 변이로 시험받지 않았다** —")
    print("   designer 산출물이 생기기 전까지 그 둘은 미검증 코드다.")
    return 0
