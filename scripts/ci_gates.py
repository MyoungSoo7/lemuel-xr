...
이 초록이 말하지 않는 것:
    ① 게이트가 통과한다 — 아니다. 러너 20개 중 14개가 아직 rc≠0 이다(2026-08-20;
...
def runners() -> list[tuple[str, list[str], str]]:
    ...
    # 화면이 본문을 열게 되자 그 옆 문장의 호칭이 보였다 — 하나님을 "신" 으로 부르는
    # 노출 텍스트가 남아 있다. 이 러너는 **판정하지 않고 센다.** 어느 호칭이 옳은지
    # 정해진 적이 없고, 검사기가 한쪽에 FAIL 을 매기면 그 선택이 결정 없이 규약이
    # 되기 때문이다. 그래서 두 축 다 BLOCKED 다 — 그래도 숫자는 기준선에 박히므로
    # 더 나빠지면 드리프트로 뜬다. 결정이 나면 축을 PASS/FAIL 로 바꾼다.
    out.append(("divine-title:all", ["scripts/divine_title_check.py"], "text"))
    return out
