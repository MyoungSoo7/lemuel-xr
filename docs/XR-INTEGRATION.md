...
- 어느 미션이 AR 을 여는지는 코드가 아니라 설정이 정한다 —
  ...
  도메인은 여전히 모드를 모른다. **룻은 2026-08-20 에 이 목록에서 뺐다** — 아래 §10.3
  각주 참조. 목록에 다시 넣어도 시드가 없어 404 다.
...
| job     | ✅  | ✅   | 시드 15개 (3기기 × 5씬)                                 |
| ruth    | ✅  | ❌\* | AR 시드 없음 — 2026-08-20 에 15개를 지웠다              |

\* **룻만 AR 이 닫혀 있다. 저작이 덜 된 게 아니라 결정이다.** AR 패스스루는 씬을
사용자의 **실제 방** 에 놓는다. 룻 Scene 4(등급 C · 밤 · 권력 비대칭)를 그렇게 놓는 것이
VR 과 같은 노출인지에 답할 사람이 없었고, 답 없이 열지 않는 쪽을 골랐다
(`docs/RUTH-RUNTIME-SIGNOFF.md`). 라합의 ❌(아직 못 만들었다)와 글자는 같지만
되돌리는 조건이 다르다.

닫는 방식은 목록 한 줄이 아니다. `application.yml` 의 `ar-enabled-missions` 기본값과
`scripts/gen_ar_manifests.py` 의 `AR_MISSIONS` 에서 빼는 동시에 **이미 생성해 둔 룻 AR
시드 15개를 지웠다.** 목록만 고쳐 두면 환경변수 `XR_AR_MISSIONS` 한 줄로 되살아나고,
그때 에셋이 이미 있으면 아무 저항 없이 열리기 때문이다. `RuntimeExposureSignoffTest` 가
설정과 시드 **둘 다** 를 보고, `ArManifestSeedResourcesTest` 가 시드가 슬쩍 돌아오면
빨개진다. 되살리는 절차는 대장 문서에 적어 뒀다.

**미션 자체는 2026-08-20 에 열렸다** — `Character` enum 에 `RUTH("ruth")` 가 들어가면서
`scenarios/ruth.yml` 이 실제로 로드된다. 처음 이 게이트는 인간 사인오프 2인(신학 ·
정신건강·안전)을 요구했으나 그 요구는 철회했고, 지금 `RuntimeExposureSignoffTest` 가
...
`script_ar_anchor_placement` 를 더한다. 총량은 VR 3,256MB(8미션) → AR 1,863MB
(**7미션** × 3기기 × 전 씬 합계 — 룻 AR 은 위 각주대로 지웠다. 요셉만 보면 376MB → 219MB).

묵상 씬(`env_meditation_*` — 씬 번호로 요셉 5 · 모세 6 · 다윗 6 · 예수 5·7 ·
엘리야 2·4 · 솔로몬 2·4 · 욥 1·4. 룻 4 도 묵상 씬이지만 룻은 AR 자체가 없다)만 예외다. 환경을 지우면 남는 게
...
> `xr_mode=ar` 계약과 매니페스트가 들어왔다(§10.3). 룻은 만들어 뒀다가 **판단이 없어서 도로 닫았다**
> — 같은 절의 각주를 보라. 여전히 보류인 건 위 표의 두 가지 — 세션을 넘어
