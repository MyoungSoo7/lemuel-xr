...
#   ⚠️ 자구는 「고른」 것이 아니라 **남은** 것이다. 주어가 또렷한 4:18-22 판본은
#   「오벳」이 AC 33 금지 토큰이라 못 쓰고, 그 목록을 무르게 하는 쪽은 택하지
#   않았다. 그 대가로 「그는」의 선행사가 화면에 없다(4:16·4:17 전반은 E16·E17).
#   미해소다. 자세한 오독 방향은 `content/ruth/scene5.yml` 의 note 에 있다.
genealogy_caption:
  id: subtitle_ruth_4_17b
  scene_id: 5
  text_ko: 그는 다윗의 아버지인 이새의 아버지였더라
  verse_ref: 룻 4:17 중
closing_lines:
