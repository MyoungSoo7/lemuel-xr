...
(네 번째 줄로 `시작된다 -> sijagdoenda | 시작뙨다 -> sijagttoenda` 도 적혀 있었다. 그건
경음화라서 지금은 적용하지 않는다 — 바로 아래 절 참조.)
...
## 경음화(23항)는 일부러 적용하지 않는다 — 되돌리지 말 것

경음화는 한국어 **정서법이 절대 적지 않는** 규칙이다(`먹다`를 `먹따`로 쓰지 않는다). 그래서
발음형으로 적어 넘기면, XTTS 가 학습한 "철자의 로마자화" 분포에 없는 문자열이 만들어진다.
평파열음 받침(d/g/b) 바로 뒤에 된소리(tt/kk/pp/ss/jj)가 붙는 자음 덩어리가 그것이다:

    지났다  철자 jinassda    | 경음화까지 적으면 jinadtta
    같고    철자 gatgo       |                gadkko
    기억도  철자 gi-eogdo    |                gi-eogtto
    덮었다  철자 deop-eossda |                deopeodtta

낭독 코퍼스 93 조각(13,656자)을 실제 파드에서 로마자화해 이 덩어리를 센 값(2026-08-15):

    원문 철자             8 회
    경음화까지 적용      200 회   ← 25 배
    경음화만 뺀 것        23 회   (21 회는 `dj`, 철자에도 원래 있는 조합)

즉 XTTS 에 들어가는 문자열의 25 배가 학습 분포 밖이 된다. 이 모듈이 처음부터 "로마자를 직접
만들지 말라"고 적어 둔 것과 같은 실수를, 우리가 경음화로 저지르고 있었다. 실제로 사용자가
`/moses` 에서 "알 수 없는 소리에 잡음이 섞인다"고 보고한 자리가 이 덩어리들이 몰린 문장이다.

연음·ㅎ탈락·비음화·유음화는 그대로 둔다. 이 규칙들의 결과는 정서법이 실제로 쓰는 글자
조합이라(`거름도`·`아는`·`궁물`·`실라`) 분포 안에 남는다. 위 표의 8 회 → 23 회가 그 값이다.
