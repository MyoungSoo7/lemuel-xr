 ...
 *
 * <p><b>싱글턴 컨테이너 패턴</b>: 컨테이너를 static 블록에서 한 번만 start 하고 stop 하지 않는다
 * (JVM 종료 시 Ryuk 이 정리). {@code @Testcontainers}/{@code @Container} 로 클래스별 lifecycle
 * 을 맡기면 IT 클래스가 2개 이상일 때 첫 클래스 종료 시 컨테이너가 내려가 두 번째 클래스가
 * "Connection refused" 로 깨진다 — 이를 피하기 위해 수동 싱글턴으로 공유한다.</p>
 */
